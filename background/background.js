...
async function addTicket({ ticketNumber, ticketType }) {
    const { tickets, settings } = await chrome.storage.local.get(['tickets', 'settings']);
    let executionTime = new Date().getTime() + calculateExecutionTimeDelta(settings);
    if (tickets.length) {
        executionTime = Math.max(...tickets.map((t) => t.executionTime)) + calculateExecutionTimeDelta(settings);
    }
    const newTicket = {
        ticketNumber,
        ticketType,
...
async function processTickets() {
    const { tickets, isProcessing } = await chrome.storage.local.get(['tickets', 'isProcessing']);
    ...
    for (let ticket of tickets) {
        if (ticket.status === 'pending') {
            ...
            await sendMessage('popup:updateTicketList');
            if (!(await chrome.storage.local.get(['isProcessing'])).isProcessing) break; // Stop processing if toggled off
        }
    }
    await chrome.storage.local.set({ tickets });
}
...
function calculateExecutionTimeDelta(settings) {
    ...
    const randomAdjustment = Math.floor(Math.random() * (randomMax - randomMin + 1)) + randomMin;
